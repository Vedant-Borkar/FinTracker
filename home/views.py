...
def index(request):
    if request.session.has_key('is_logged'):
        ...
        addmoney_info1 = Addmoney_info.objects.filter(user=user).order_by('-Date')
        ...
        avail= user.userprofile.Savings+addmoney_info.sum1
        addmoney_info.avail= abs(avail)
        addmoney_info.x = abs(x)
        addmoney_info.y = abs(y)
        availbal={
            'availbal':addmoney_info
        }
    #if request.session.has_key('is_logged'):
        return render(request,'home/index.html',availbal)
...
def stats(request):
    if request.session.has_key('is_logged') :
        ...
        avail= user1.userprofile.Savings+addmoney_info.sum1
        if x<0:
